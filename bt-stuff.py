...
def find_device_in_objects(objects, device_address, adapter_pattern=None):
    ...
    path_prefix = ''
    if adapter_pattern:
        if not isinstance(adapter_pattern, types.StringTypes):
...
def main(args=None):
    ...
    if not opts.device_all:
        devs = [next(iter(find_adapter(opts.device)))]
    else:
        if opts.call != 'server':
    ...
    if opts.systemd:
        ...
        if wd_interval:
            ...
            wait_iter.sd_wdt, wait_iter.timeout = False, wait_iter_noop
        wait_iter.sd_ready = False
    else:
        wait_iter = lambda: time.sleep(wait_iter_noop)
    ...
    if opts.call == 'server':
        ...
        try:
            dev_remote.ConnectProfile(opts.uuid)
        except:
